...
// Verifica se o tema Brite está aplicado e os links do topo existem

test('Layout público usa tema Brite e links no topo', async ({ page }) => {
  await page.goto('/')
  ...
  await expect(bootswatchLink).toHaveAttribute(
    'href',
    /bootswatch@5.3.8\/dist\/brite\/bootstrap\.min\.css/,
  )

  // Verifica se os links do topo existem
  await expect(page.locator('a', { hasText: 'Validar' })).toBeVisible()
...
test.describe('Layout admin usa tema Brite e navbar com texto preto', () => {
  ...
  test('navbar admin com tema e links corretos', async ({ page }) => {
    ...
    // Os links devem ter texto preto (4 links: Certificação, Eventos, Administração, menu do usuário)
