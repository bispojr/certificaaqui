...
async function createUserAndToken(perfil, eventoId = null) {
  ...
  const token = jwt.sign({ id: user.id, perfil: user.perfil, evento_id: user.evento_id }, JWT_SECRET);
  return { user, token };
}
...
describe('Proteção das rotas de gestão', () => {
  ...
  beforeAll(async () => {
    ...
    const evento1 = await Evento.create({ nome: 'Evento 1', codigo_base: 'ABC', ano: 2026 });
    const evento2 = await Evento.create({ nome: 'Evento 2', codigo_base: 'DEF', ano: 2026 });
