...
- [ ] **III. Layered MVC** — Feature respects `models → services → controllers → routes → middlewares` dependency direction.
- [ ] **IV. Soft Delete** — Any entity removal uses logical delete (Sequelize Paranoid); no `force: true` in production paths.
- [ ] **V. Security/RBAC** — Protected routes use JWT middleware; RBAC middleware applied before business logic; OWASP Top 10 checked.
